# BLEAutoUnlock —— 睡眠唤醒蓝牙近场检测工具

Windows 10/11（x64）+ Python 3.10+ 下的本地工具：电脑从睡眠（Sleep）唤醒后停留在锁屏界面时，通过检测指定 iPhone 的蓝牙信号强度（RSSI），靠近时触发解锁、离开后触发锁定。所有逻辑全部本地实现，不依赖任何付费 API。
