# BLEAutoUnlock —— 睡眠唤醒蓝牙自动锁屏工具

Windows 10/11（x64）+ Python 3.10+ 下的本地工具：电脑从睡眠（Sleep）唤醒后，通过检测指定 iPhone 的蓝牙信号强度（RSSI），在设备离开或信号变弱时自动锁屏。所有逻辑全部本地实现，不依赖任何付费 API。本工具仅提供自动锁屏功能。

> 明确排除：本程序不处理"关机后首次开机"的登录界面，仅限睡眠/休眠唤醒后的自动锁屏。

## 工作原理

```text
BLE 扫描(bleak) ──> 设备识别(resolver, IRK) ──> RSSI 状态机 ──> 锁屏(actions)
      │                                                  │
      └── 睡眠唤醒恢复(异常重连 + WM_POWERBROADCAST) ───────┘
```

- 持续扫描 BLE 广播，用 IRK 解析 iPhone 的随机可解析私有地址（RPA，MAC 每 15 分钟变化一次）。
- `RSSI < lock_rssi` 或设备消失，连续 `miss_count_before_lock` 次扫描**且累计超过 `lock_min_elapsed` 秒** → 锁屏（误判防御）。
- 设备信号在阈值附近波动时保持当前状态，避免频繁触发锁屏。
...
├── actions.py           # 锁屏 Windows API 封装（LockWorkStation）
...
| `miss_count_before_lock` | `5` | 连续多少次扫描不到目标才考虑锁屏 |
| `lock_min_elapsed` | `10` | 设备消失后累计最少经过的时间（秒），与次数同时满足才锁屏 |
...
服务无控制台窗口。**注意**：Windows 服务默认运行在 Session 0，无法访问用户的交互桌面，建议使用"用户登录自启"方式运行。
...
使用 `pythonw.exe` 可无控制台窗口运行；任务计划程序中选择"使用最高权限运行"可提升功能的稳定性。
...
- 蓝牙阈值：近场 RSSI、锁屏 RSSI（dBm）
- 判定时间：扫描间隔、单次扫描时长、靠近持续时长、离开累计时长、连续未检测次数
- 高级选项：IRK 解析方法（ble_standard / legacy_hmac）、prand 位置
- 二级菜单：帮助(H) → **RSSI 距离对照表**，查看 Apple iBeacon 官方分级与不同距离对应的参考 RSSI，便于校准锁屏阈值
- 操作：保存配置、开始/停止监听，以及实时 RSSI、会话状态与运行日志

界面修改后点"保存配置"写入 `config.json`；点"开始监听"后后台运行扫描状态机，锁屏判定逻辑与命令行模式完全一致。
...
| 服务模式功能受限 | 服务运行在 Session 0，无交互桌面；改用"用户登录自启"方式。 |
| RSSI 波动导致频繁锁屏 | 增大 `miss_count_before_lock` / `lock_min_elapsed`，或结合日志调整 `lock_rssi` 阈值。 |

## 已知限制与安全说明

- 密码仅用 DPAPI 加密保存在本机，密钥与当前用户绑定；请勿分享 `config.json`。
- 蓝牙 RSSI 受环境干扰，阈值建议结合日志实测调整。
- 本程序仅处理睡眠/休眠唤醒后的自动锁屏，不处理首次开机登录界面。
